...
function resolveSkin (source, waveSkin) {
...
function laneParamsFromSkin (index, source, lane, waveSkin) {
    ...
    const skin = resolveSkin(source, waveSkin);
...
function renderSignal (index, source, waveSkin, notFirstSignal) {
    ...
    const skin = resolveSkin(source, waveSkin);
    const waveGroups = renderGroups(ret.groups, index, lane, skin);
    ...
    return insertSVGTemplate(
        ...
        renderLanes(index, content, waveLanes, ret, source, lane, skin),
        waveGroups,
        notFirstSignal
    );
