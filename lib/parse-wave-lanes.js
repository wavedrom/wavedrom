...
function parseWaveLanes (sig, lane) {
    ...
    for (x in sig) {
        ...
        tmp0[0] = sigx.name  || ' ';
        tmp0[1] = sigx.phase || 0;
        //assign the draggable boolena so that the wavelane can be rendered
        //with the draggable class
        content[content.length - 1].draggable = sigx.draggable || false;
        //passing the wave to render the gaps
        content[content.length - 1].wave = sigx.wave;
