...
function renderWaveForm (index, source, output) {
    ...
    if (source.signal) {
        ...
        // if no config.hbounds, lane.xmax+1 is used for width (max of all wave lengths plus 1 brick)
        // if config.hbounds, width = xmax_cfg-xmin_cfg may be smaller than xmax
        width  = (lane.xg + (lane.xs * (Math.min(lane.xmax + 1,lane.xmax_cfg-lane.xmin_cfg) )));
