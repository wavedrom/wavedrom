...
function parseWaveLane (text, extra, lane) {
    ...
    while (Stack.length) {
        ...
        if (Next === '<') { // sub-cycles on
            subCycle = true;
            Next = Stack.shift(); 
            // If the first character in a sub-cycle is a dot, the previous character
        ...
        if (Next === '>') { // sub-cycles off
            subCycle = false;
            Next = Stack.shift();
            // If the first character following a sub-cycle is a dot, the last encountered
            // character within the sub-cycle set must be carried over, and the next
