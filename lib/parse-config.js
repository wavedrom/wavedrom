...
function parseConfig (source, lane) {
    ...
    if (source && source.config && source.config.hbounds && source.config.hbounds.length==2) {
        ...
        if (  source.config.hbounds[0] < source.config.hbounds[1] ) {
            ...
            // TODO: do we want to base this on ticks in head or foot?  They can be different...
    ...
    if (source && source.head) {
        if (
            ...
            // if ticks defined, modify start tick by lane.xmin_cfg
    ...
    if (source && source.foot) {
        if (
            ...
            // if ticks defined, modify start tick by lane.xmin_cfg
            source.foot.tick = source.head.tick + lane.xmin_cfg;
            source.foot.tock = source.head.tock + lane.xmin_cfg;
            lane.yf0 = 20;
        }
