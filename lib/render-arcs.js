...
const getAccentColor = require('./get-accent-color.js');
...
const archer = (res, Events, arcFontSize, accentColor, bgFill) => (element) => {
    ...
    if (from && to) {
        const shapeProps = arcShape(Edge, from, to, accentColor);
        const lx = shapeProps.lx;
        const ly = shapeProps.ly;
        res.push(renderArc(Edge, from, to, shapeProps));
        if (Edge.label) {
            res.push(renderLabel({x: lx, y: ly}, Edge.label, arcFontSize, bgFill));
        }
    }
};

function renderArcs (lanes, index, source, lane, skin) {
    const arcFontSize = (source && source.config && source.config.arcFontSize)
        ? source.config.arcFontSize
        : 11;

    const bgFill = source && source.config && source.config.background || "white";

    const accentColor = getAccentColor(skin);
    ...
    if (Array.isArray(lanes)) {
        ...
        if (Array.isArray(source.edge)) {
            source.edge.map(archer(res, Events, arcFontSize, accentColor, bgFill));
        }
